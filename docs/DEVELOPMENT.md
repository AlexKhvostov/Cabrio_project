...
**Стек на хостинге:** PHP 8.1 + HTML/CSS/обычный JS, без Vue и без `npm run build` на сервере. Актуальная картина продукта и план: [CURRENT_STATUS.md](CURRENT_STATUS.md).

Ниже в этом файле ещё встречаются старые формулировки (Node, Composition API) — это черновик процесса, не описание текущего фронта.
