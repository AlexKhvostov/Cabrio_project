...
> Технические детали авторизации, сессий, ролей и идентификации описаны в [docs/AUTH_ARCHITECTURE.md](../AUTH_ARCHITECTURE.md). Здесь приведён только пользовательский сценарий.
...
7. Если пользователь не состоит в чате — роль становится `external`, показывается заглушка с инструкцией вступить в чат.
8. Проверка выполняется один раз при входе в Mini App. Переходы по разделам Telegram повторно не спрашивают. Если Telegram не ответил, доступ не запирается.
...
- Если Telegram API недоступен — роль не меняется, Mini App показывает повтор проверки и не пускает внутрь, пока нет подтверждения из чата.
- Если пользователь возвращается в чат — при следующей проверке роль обновляется, доступ восстанавливается.
...
- Пользователь: member/user/guest/admin/moderator → external (при выходе из чата)
- При возвращении в чат — роль становится guest или user (по наличию авто/главного гостя)
...
- Backend: создание/обновление пользователя, смена роли
...
- Проверка членства один раз при входе в Mini App, не на каждом разделе
...
- Пользователь возвращается в чат — роль `external` меняется на `guest`, доступ восстанавливается
