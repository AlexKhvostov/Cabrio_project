...
- Если пользователь был с ролью member и удаляет последний авто — он остаётся member. Автоматического понижения роли не происходит. Понизить роль может только администратор вручную (и только до user). См. docs/USER_ROLES.md
...
- При добавлении первого авто: если был guest, роль пользователя может измениться на user (при выборе авто/главного гостя). См. docs/USER_ROLES.md
...
- Пользователь добавляет первый авто, если был guest, роль пользователя меняется на user.
