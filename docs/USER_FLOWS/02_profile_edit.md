...
- Если роль пользователя была понижена вручную (например, до user), для возврата к member требуется повторная модерация (см. отдельный сценарий).
...
- Если роль понижена вручную (например, до user), возврат к member возможен только после повторной модерации (см. сценарий модерации).
