...
3. В секции отображаются:
   - Текущая роль пользователя (external, guest, user, member, moderator, admin)
...
## 7. Переходы ролей
- Роль пользователя (кроме admin) может быть изменена админом (например, user → member, member → moderator и т.д.)
