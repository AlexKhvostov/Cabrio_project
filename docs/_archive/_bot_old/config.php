...
function getApiUrl() {
    global $env;
    // Теперь используем отдельную переменную BACKEND_API_URL для всех запросов к backend API
    $baseUrl = $env['BACKEND_API_URL'] ?? 'http://localhost/app/backend/api';
...
$config = [
    ...
    'club_chat_id' => $env['CLUB_CHAT_ID'] ?? null,
    ...
    'messages' => [
        'start' => [
            ...
            'non_member' => "❌Я работаю только с участниками клубного чата.\n\n📱 Для доступа к функциям бота вступите в наш чат:",
...
if (empty($config['club_chat_id'])) {
    die('ERROR: CLUB_CHAT_ID not set in .env file');
}
...
function isMainChatMember($chatId) {
    $config = getConfig();
    $mainChatIds = parseIds($config['club_chat_id']);
