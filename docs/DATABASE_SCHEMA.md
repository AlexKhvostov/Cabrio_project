...
> **Пояснение:** Запись о пользователе создаётся в базе при первом обращении к боту. Пользователь с незавершённой регистрацией — это статус "guest" или "external". Только пользователи со статусом, отличным от "guest" и "external", считаются завершившими регистрацию и получают доступ к основному функционалу.
