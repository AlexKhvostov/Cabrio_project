...
> **Пояснение:** Запись о пользователе создаётся в базе при первом обращении к боту. У участников есть только **роли** (`external`, `guest`, `user`, `member`…), не статусы. Основной функционал клуба — с роли `member` и выше. Статусы (`active`, `pending`…) — у авто, событий, мест и отзывов.
...
| guide_objects            | Карточки раздела «Отзывы» (мойки, сервисы; код `guide_objects`) |
...
- Если пользователь выключает «показывать меня», строка удаляется сразу (`DELETE /api/user-locations`).
- Для отображения на карте используются только записи, обновлённые не более N минут назад (`map_live_time_min`, по умолчанию 60).
...
> Фото карточки: запись с максимальным id в photos с entity_type = 'guide_object' и entity_id = guide_objects.id.
> В меню раздел называется **«Отзывы»** (`pages/services.php`). Тип и вид в форме не спрашиваем — вместо них ярлыки (`labels`).
