                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    ...
                    <div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="plate" value="A123BC" required>
                            <div class="optional">Формат: A123BC</div>
                        </div>
                    </div>
                </div>
                
                <button type="button" onclick="sendRequest()">🚀 Проверить номер</button>
            </form>
        </div>
    ...
    <script>
        ...
        async function sendRequest() {
            ...
            document.getElementById('response').innerHTML = 
                '<div class="info">⏳ Проверка номера...</div>';
            
            try {
                const response = await fetch('../../api/ocr/check.php', {
                    method: 'POST',
                ...
                if (result.found) {
                    responseDiv.innerHTML = `
                        <div class="success">
                            ...
                            <p><strong>Можно оставить визитку:</strong> ${result.can_leave_card ? 'Да' : 'Нет'}</p>
                            <hr>
                    ...
                    responseDiv.innerHTML = `
                        <div class="info">
                            ...
                            <p><strong>Сообщение:</strong> ${data.result.message}</p>
                            <p><strong>Искали номер:</strong> ${result.plate}</p>
                            <p><strong>Можно оставить визитку:</strong> ${result.can_leave_card ? 'Да' : 'Нет'}</p>
                            <hr>
                            <h4>📋 Полный ответ сервера:</h4>
                            <pre>${JSON.stringify(data, null, 2)}</pre>
