...
class UpdateCarEndpoint extends ApiHandler {
    protected function process() {
        ...
        // Новая логика: поддержка status (строковый код) и status_id (числовой id)
        if (isset($data['status_id'])) {
            $fields['status'] = (int)$data['status_id'];
        } elseif (isset($data['status'])) {
            // Получаем id статуса по коду
            $statusCode = $data['status'];
            $stmt = $pdo->prepare('SELECT id FROM ref_statuses WHERE code = ? AND entity_type = ?');
            $stmt->execute([$statusCode, 'car']);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!$row) {
                return $this->error(400, 'INVALID_STATUS', 'Неизвестный код статуса: ' . $statusCode);
            }
            $fields['status'] = (int)$row['id'];
        }
        if (isset($data['model'])) {
