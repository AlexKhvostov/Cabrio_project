...
class GroupPhotoCheckHandler {
    ...
    public function execute($message, $userSyncResult = null) {
        try {
            ...
            if ($role === 'external') {
                writeToLog('GroupPhotoCheckHandler: пользователь не в клубе, игнорируем');
                return;
            }
    ...
    private function processPhotoWithNewAPI($base64_image, $userSyncResult) {
        try {
            ...
            if (!$token) {
                writeToLog('GroupPhotoCheckHandler: не удалось получить токен');
            ...
            writeToLog('GroupPhotoCheckHandler: отправка запроса на новый API', [
                'url' => $api_url,
            ...
            writeToLog('GroupPhotoCheckHandler: ответ от нового API', [
                'http_code' => $http_code,
                'response' => $response,
