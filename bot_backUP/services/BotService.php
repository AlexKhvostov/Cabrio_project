...
class BotService {
    ...
    /**
     * Получает JWT токен для пользователя через новый API
     * @param array $userSyncResult — результат синхронизации профиля
     * @return string|null — JWT токен или null
     */
    public function getUserToken($userSyncResult) {
        try {
            if (!$userSyncResult || !isset($userSyncResult['telegram_id'])) {
                writeToLog('getUserToken: нет данных пользователя');
                return null;
            }
            
            $api_url = getApiUrl() . '/api/auth/telegram';
            
            // Формируем данные для получения токена
            $telegramData = [
                'id' => $userSyncResult['telegram_id'],
                'first_name' => $userSyncResult['first_name'] ?? null,
                'last_name' => $userSyncResult['last_name'] ?? null,
                'username' => $userSyncResult['username'] ?? null,
                'language_code' => $userSyncResult['language_code'] ?? 'ru'
            ];
            
            // Создаем initData (имитация Telegram WebApp)
            $initData = $this->createInitData($telegramData);
            
            $request_data = [
                'initData' => $initData
            ];
            
            writeToLog('getUserToken: запрос токена', [
                'url' => $api_url,
                'telegram_id' => $userSyncResult['telegram_id']
            ]);
            ...
            writeToLog('getUserToken: ответ от API', [
                'http_code' => $http_code,
                'response' => $response,
            ...
            if ($http_code === 200) {
                ...
                if ($result && isset($result['success']) && $result['success']) {
                    ...
                    if ($token) {
                        writeToLog('getUserToken: токен получен успешно');
                        return $token;
                    }
                }
            }
            
            writeToLog('getUserToken: ошибка получения токена', [
                'http_code' => $http_code,
                'curl_error' => $curl_error
            ]);
            ...
            writeToLog('getUserToken: исключение', [
                'error' => $e->getMessage()
            ]);
            return null;
        }
    ...
    /**
     * Создает initData для имитации Telegram WebApp
     * @param array $userData — данные пользователя
     * @return string — initData строка
     */
    private function createInitData($userData) {
        // Создаем базовую структуру initData
        $queryParams = [
            'user' => json_encode($userData),
            'auth_date' => time(),
            'query_id' => 'bot_' . uniqid()
        ];
        ...
        // Создаем подпись (в реальности это должна быть подпись от Telegram)
        $hash = hash_hmac('sha256', $dataCheckString, 'bot_secret_key', false);
     ...
     * Вызывает новый API для создания/обновления пользователя
     * @param array $user — массив с данными Telegram-пользователя
    ...
    public function syncTelegramRequestorProfile($user) {
        try {
            ...
            // Формируем данные пользователя для нового API
            $userData = [
            ...
            $token = $this->getUserToken(['telegram_id' => $user['id']]);
            
            $api_url = getApiUrl() . '/api/users';
            
            writeToLog('syncTelegramRequestorProfile: запрос к новому API', [
                'url' => $api_url,
                'user_data' => $userData
            ]);
            
            // Отправляем запрос на создание/обновление пользователя
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $api_url);
            curl_setopt($ch, CURLOPT_POST, 1);
            ...
            writeToLog('syncTelegramRequestorProfile: ответ от нового API', [
                'http_code' => $http_code,
                'response' => $response,
                'curl_error' => $curl_error
            ]);
            ...
            if ($http_code === 200 || $http_code === 201) {
                ...
                if ($result && isset($result['success']) && $result['success']) {
                    ...
                    // Добавляем совместимость со старой структурой
                    if (isset($userData['role'])) {
                        $userData['role_string'] = $userData['role'];
                    }
                    
                    writeToLog('syncTelegramRequestorProfile: пользователь синхронизирован', [
            ...
            writeToLog('syncTelegramRequestorProfile: ошибка синхронизации', [
                'http_code' => $http_code,
                'curl_error' => $curl_error
            ]);
            ...
            writeToLog('syncTelegramRequestorProfile: исключение', [
                'error' => $e->getMessage()
            ]);
            return null;
        }
